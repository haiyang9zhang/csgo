...
def yolo_head(feats, anchors, num_classes, input_shape):
    ...
    # 将feats的最后一维255展开，将anchors与其他数据分离
    feats = K.reshape(
...
def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape):
    # feat:输出的shape,如(?,13,13,255)，anchors：该shape对应的三个anchor box
    # box_xy:box中心坐标（0-1相对位置），box_wh:box宽高（0-1相对位置），box_confidence:框中物体置信度，box_class_probs:类别置信度
    box_xy, box_wh, box_confidence, box_class_probs = yolo_head(feats,
        anchors, num_classes, input_shape)
    # 将box_xy, box_wh转为真实坐标，输出boxes是(?,13,13,3,4),4是(y_min,x_min,y_max,x_max)
    boxes = yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape)
    # 将不同网格的值转换为框的列表，(?,13,13,3,4)->(?,4)
    boxes = K.reshape(boxes, [-1, 4])

    # 框的得分=框的置信度*类别置信度
    box_scores = box_confidence * box_class_probs
    box_scores = K.reshape(box_scores, [-1, num_classes])    # 将框的得分展平为(?,80)，?为框的数目
    return boxes, box_scores

# 图片预测
def yolo_eval(yolo_outputs,  # 模型输出，格式如下：（?,13,13,255），（?,26,26,255），（?,52,52,255），?：batch size
              anchors,
    ...
    for c in range(num_classes):
        ...
        class_box_scores = tf.boolean_mask(box_scores[:, c], mask[:, c])  # 通过掩码mask和类别筛选box_scores

        # 运行非极大值抑制，得到通过抑制的索引
        nms_index = tf.image.non_max_suppression(
            class_boxes, class_box_scores, max_boxes_tensor, iou_threshold=iou_threshold)

        class_boxes = K.gather(class_boxes, nms_index)     # 检索张量class_boxes中索引为nms_index的元素
        class_box_scores = K.gather(class_box_scores, nms_index)  # 检索张量class_box_scores中索引为nms_index的元素
        classes = K.ones_like(class_box_scores, 'int32') * c  # K.ones_like实例化与张量class_box_scores具有相同形状的全1变量
